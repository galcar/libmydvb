...
void mydvb_engine_init (MYDVB_ENGINE *engine) {
	...
	engine->tunes = dyn_array_new (sizeof (MYDVB_TUNE *));
	...
	for (adapter=0; adapter < MYDVB_MAX_ADAPTERS; adapter ++) {
		for (device=0; device < MYDVB_MAX_DEVICES; device ++) {
			...
			if (mydvb_tune_init (tune, adapter, device) == 0){ // ok
				...
				dyn_array_add (engine->tunes, &tune);
 ...
 * the dvb main loop
 */
void mydvb_main_loop_engine (MYDVB_ENGINE *engine) {
		...
		if (r > 0) {
			...
			for (j=0; j < engine->poll_len; j++) {
				...
				if (engine->pfd[j].revents & POLLPRI || engine->pfd[j].revents & POLLIN) {
					...
					for (k=0; k < dyn_array_get_size (engine->tunes); k++) {
						MYDVB_TUNE *tune = *((MYDVB_TUNE **) dyn_array_get_data (engine->tunes, k));
						...
						/* check for frontend event */
							...
							if (ioctl(tune->fd, FE_GET_EVENT, &f_event) == 0) {
								...
								if (f_event.status & FE_TIMEDOUT) {
									...
									if (tune->status == TUNE_STATUS_TRYING) {

										tune->status = TUNE_STATUS_FAILED;

										// fire event
										event.type = MYDVB_TUNE_EVENT;

										event.tune.status = tune->status;
							...
							for (i = 0; i < mydvb->sct_fd_len; i++) {
								...
								if (engine->pfd[j].fd == mydvb->sct_fd[i].fd) {

									/* decode table */

									mydvb_read_table (mydvb, engine->pfd[j].fd, buffer);
